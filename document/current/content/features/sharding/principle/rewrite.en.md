...
As shown in the picture, the user wants to acquire the second and the third piece of data ordered by score in both tables, and they are supposed to be `95` and `90`. Since the executed SQL can only acquire the second and the third piece of data from each table, i.e., `90` and `80` from t_score_0, `85` and `75` from t_score_1.In result merger, it can only merge from `90`, `80`, `85` and `75` already acquired, so the right result cannot be acquired anyway.
...
The latter the offset position is, the lower the efficiency of using LIMIT pagination will be. There are many ways to avoid using LIMIT as pagination method, such as constructing a secondary index to record line record number and line offset amount, or using the last ID of last pagination data as the pagination method in the next query.

When revising pagination information, if users write SQL with placeholder, he only needs to rewrite parameter list rather than SQL itself.
