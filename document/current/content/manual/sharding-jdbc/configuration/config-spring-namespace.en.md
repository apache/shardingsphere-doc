...
###### SNOWFLAKE
  ...
  | max.vibration.offset(?)                             |    int     |    optional                 | The max upper limit value of vibrate number, range `[0 , 4096)`, the default value is `1`. Notice: If you need to use its generate value as sharding value, it is recommended to configure this property. Because by default configuration , the algorithm generates key mod `2^n` (`2^n` is usually the amount of your tables or databases) in different milliseconds and the result is always `0` or `1`. To prevent the above sharding problem, it is recommended that you configure this property, its value is `(2^n)-1` |
...
###### LEAF_SEGMENT
  ...
  | leaf.key(?)                           | String     |    required                 | The key which can be used to get the max segment id.                                                                                                                                         |
  ...
  | registry.center.digest(?)             | String     |    optional                 | Connect to authority tokens in registry center; default indicates no need for authority confirmation.                                                                                        |
...
###### LEAF_SNOWFLAKE
  ...
  | max.tolerate.time.difference.milliseconds(?)      | long       |    optional                 | The max time difference between native and registry center in milliseconds,the default value is `10000`. If it exceeds the configuration time, an exception is thrown when the job starts.                                      |
  | registry.center.digest(?)                         | String     |    optional                 | Connect to authority tokens in registry center; default indicates no need for authority confirmation.                                                                                                                           |
  | registry.center.type(?)                           | String     |    optional                 | The type of registry center , the default value is `zookeeper`.                                                                                                                                                                 |
  | max.vibration.offset(?)                           | int        |    optional                 | The max upper limit value of vibrate number, range `[0 , 4096)` ,the default value is `1`. Notice: If you need to use its generate value as sharding value, it is recommended to configure this property. Because by default configuration , the algorithm generates key mod `2^n` (`2^n` is usually the amount of your tables or databases) in different milliseconds and the result is always `0` or `1`. To prevent the above sharding problem, it is recommended that you configure this property, its value is `(2^n)-1` |
