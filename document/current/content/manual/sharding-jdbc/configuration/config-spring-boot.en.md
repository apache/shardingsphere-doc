...
spring.shardingsphere.sharding.tables.t_order_item.encryptor.type=MD5
spring.shardingsphere.sharding.tables.t_order_item.encryptor.columns=status
spring.shardingsphere.sharding.tables.t_order_encrypt.actual-data-nodes=ds_$->{0..1}.t_order_encrypt_$->{0..1}
spring.shardingsphere.sharding.tables.t_order_encrypt.table-strategy.inline.sharding-column=order_id
spring.shardingsphere.sharding.tables.t_order_encrypt.table-strategy.inline.algorithm-expression=t_order_encrypt_$->{order_id % 2}

spring.shardingsphere.sharding.tables.t_order_encrypt.encryptor.type=QUERY
spring.shardingsphere.sharding.tables.t_order_encrypt.encryptor.columns=encrypt_id
spring.shardingsphere.sharding.tables.t_order_encrypt.encryptor.assistedQueryColumns=query_id
```
...
```properties
spring.shardingsphere.sharding.tables.<logic-table-name>.encryptor.type= #Type of encryptor，use user-defined ones or built-in ones, e.g. MD5/AES 
spring.shardingsphere.sharding.tables.<logic-table-name>.encryptor.columns= #Column name of key generator      
spring.shardingsphere.sharding.tables.<logic-table-name>.encryptor.assistedQueryColumns= #assistedColumns for query，when use ShardingQueryAssistedEncryptor, it can help query encrypted data
spring.shardingsphere.sharding.tables.<logic-table-name>.encryptor.props..<property-name>= #Properties, e.g. `aes.key.value` for AES encryptor  
```
...
spring.shardingsphere.sharding.orchestration.name= #Data orchestration instance name
spring.shardingsphere.sharding.orchestration.overwrite= #Whether to overwrite local configurations with registry center configurations; if it can, each initialization should refer to local configurations
spring.shardingsphere.sharding.orchestration.registry.type= #Registry center type. Example:zookeeper
spring.shardingsphere.sharding.orchestration.registry.server-lists= #The list of servers that connect to registry center, including IP and port number; use commas to separate
spring.shardingsphere.sharding.orchestration.registry.namespace= #Registry center namespace
spring.shardingsphere.sharding.orchestration.registry.digest= #The token that connects to the registry center; default means there is no need for authentication
spring.shardingsphere.sharding.orchestration.registry.operation-timeout-milliseconds= #The millisecond number for operation timeout; default value: 500 milliseconds
spring.shardingsphere.sharding.orchestration.registry.max-retries= #Maximum retry time after failing; default value: 3 times
spring.shardingsphere.sharding.orchestration.registry.retry-interval-milliseconds= #Interval time to retry; default value: 500 milliseconds
spring.shardingsphere.sharding.orchestration.registry.time-to-live-seconds= #Living time of temporary nodes; default value: 60 seconds
spring.shardingsphere.sharding.orchestration.registry.props= #Customize registry center props.
```
