...
### 数据脱敏
```properties
spring.shardingsphere.sharding.tables.<logic-table-name>.encryptor.type= #加解密器类型，可自定义或选择内置类型：MD5/AES 
spring.shardingsphere.sharding.tables.<logic-table-name>.encryptor.columns= #加解密字段
spring.shardingsphere.sharding.tables.<logic-table-name>.encryptor.assistedQueryColumns= #辅助查询字段，针对ShardingQueryAssistedEncryptor类型的加解密器进行辅助查询  
spring.shardingsphere.sharding.tables.<logic-table-name>.encryptor.props..<property-name>= #属性配置, 比如AES算法的KEY属性：aes.key.value 
```
### 数据治理
...
spring.shardingsphere.sharding.orchestration.name= #数据治理实例名称
spring.shardingsphere.sharding.orchestration.overwrite= #本地配置是否覆盖注册中心配置。如果可覆盖，每次启动都以本地配置为准
spring.shardingsphere.sharding.orchestration.registry.type= #配置中心类型。如：zookeeper
spring.shardingsphere.sharding.orchestration.registry.server-lists= #连接注册中心服务器的列表。包括IP地址和端口号。多个地址用逗号分隔。如: host1:2181,host2:2181
spring.shardingsphere.sharding.orchestration.registry.namespace= #注册中心的命名空间
spring.shardingsphere.sharding.orchestration.registry.digest= #连接注册中心的权限令牌。缺省为不需要权限验证
spring.shardingsphere.sharding.orchestration.registry.operation-timeout-milliseconds= #操作超时的毫秒数，默认500毫秒
spring.shardingsphere.sharding.orchestration.registry.max-retries= #连接失败后的最大重试次数，默认3次
spring.shardingsphere.sharding.orchestration.registry.retry-interval-milliseconds= #重试间隔毫秒数，默认500毫秒
spring.shardingsphere.sharding.orchestration.registry.time-to-live-seconds= #临时节点存活秒数，默认60秒
spring.shardingsphere.sharding.orchestration.registry.props= #配置中心其它属性
```
